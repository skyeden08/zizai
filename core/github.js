...
export function createGitHubModule({ auth, config } = {}) {
  ...
  return createModule({ id: "github", capabilities: ["read", "write"], ready: async () => Boolean(await auth.call("getGithubToken")), actions: {
    read: async path => { const data = await request(path); return { path, sha: data.sha, content: base64ToUtf8(data.content.replace(/\n/g, "")) }; },
    write: async (path, content, message = "ZIZAI 更新 GitHub") => { let sha; try { sha = (await request(path)).sha; } catch (error) { if (!String(error.message).includes("404")) throw error; } const data = await request(path, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message, content: utf8ToBase64(content), ...(sha ? { sha } : {}) }) }); return { path, sha: data.content?.sha }; }
  }});
}
