...
export function createBridgeModule({ registry } = {}) {
  ...
  return createModule({ id: "bridge", capabilities: ["dispatch"], ready: () => true, actions: {
    parse,
    dispatch: async request => { if (!request) return null; if (request.error) throw new Error(request.error); return registry.call(request.module || "github", request.action, request.path, request.content || "", request.message || "ZIZAI 模組寫入"); }
  }});
}
