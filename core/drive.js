...
export function createDriveModule({ auth, config, onStatus } = {}) {
    ...
    return new Promise((resolve, reject) => {
      tokenClient.callback = async response => {
        ...
        expiryTimer = setTimeout(() => {
          auth.call("setDriveToken", null);
          onStatus?.(false, "備份逾時");
        }, Math.max(1, (response.expires_in || 3600) - 60) * 1000);
        onStatus?.(true, "密鑰／備份已連線");
  ...
  return createModule({
    id: "drive",
    capabilities: ["authorize", "read", "write", "upload", "delete"],
    lifecycle: { beforeReady: ["authorize"] },
    ready: async () => Boolean(await getToken()),
    actions: {
      authorize: request,
      find,
      readJson: async name => {
        const files = await find(name);
        if (!files[0]) return null;
        return (await (await api(`https://www.googleapis.com/drive/v3/files/${files[0].id}?alt=media`)).json());
      },
      writeJson: async (name, value) => {
        const files = await find(name);
        const blob = new Blob([JSON.stringify(value)], { type: "application/json" });
        if (files[0]) {
          return (await api(`https://www.googleapis.com/upload/drive/v3/files/${files[0].id}?uploadType=media`, {
            method: "PATCH",
      ...
      upload: async file => {
        const form = new FormData();
        form.append("metadata", new Blob([JSON.stringify({ name: file.name, mimeType: file.type || "application/octet-stream" })], { type: "application/json" }));
        form.append("file", file);
        return (await api("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id", { method: "POST", body: form })).json();
      },
