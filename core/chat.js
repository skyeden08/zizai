...
export function createChatModule({ registry, ui, state } = {}) {
  return createModule({ id: "chat", capabilities: ["send", "history"], ready: async () => registry.isReady("gemini"), actions: {
    send: async text => {
      if (!text?.trim()) return null;
      const history = [{ role: "user", text: text.trim() }];
      await ui.call("addBubble", "user", text.trim());
      for (let i = 0; i < 3; i++) {
        ...
        try {
          ...
          history.push({ role: "model", text: reply }, { role: "user", text: `（ZIZAI 模組執行錯誤）${error.message}` });
        }
      }
